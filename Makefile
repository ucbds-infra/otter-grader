...
docker-test-prepare: 
	...
	printf "\nRUN export DOCKER_VERSION_STRIPPED=${DOCKER_VERSION//-[0-9]*/}" >> test-Dockerfile
	printf "\nRUN cd /tmp && curl -sSL -O https://download.docker.com/linux/static/stable/x86_64/docker-${DOCKER_VERSION_STRIPPED}.tgz && tar zxf docker-${DOCKER_VERSION}.tgz && mv ./docker/docker /usr/local/bin && chmod +x /usr/local/bin/docker && rm -rf /tmp/*" >> test-Dockerfile
