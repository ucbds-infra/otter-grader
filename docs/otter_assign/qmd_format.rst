...
R Markdown, and Quarto master documents. The format for using Otter Assign with R is very similar to the 
Python format with a few important differences. The main difference is that, where in the notebook
format you would use raw cells, in Quarto files you wrap what would normally be in a raw cell in
an ``otter`` code block.

For example, a ``# BEGIN TESTS`` cell in Quarto looks like:
...
As with Python, Otter Assign for Quarto also allows you to specify various assignment generation 
arguments in an assignment config block:
...
Here is an example question in an Otter Assign-formatted Quarto question:
...
zero or more test code blocks. The blocks should be wrapped in ``otter`` blocks following the same
...
portions of lines with pre-specified prompts. The format for solution cells in Quarto files is the same 
...
provided holds for Quarto files.
...
To delimit a markup solution to a manual question, wrap the solution in a ``SOLUTION`` block:
...
To use a custom Markdown prompt, include a ``PROMPT`` block with a solution block:
